...
import com.creawor.addcamera.app.BaseFragment;
...
public class BaseDialogFragment extends DialogFragment {
    ...
    private static final String EXTRA_DIALOG_CANCEL_ABLE_KEY = "extra_dialog_cancelable";
    ...
    /**
     * 从bundle中解析参数，args有可能来自fragment被系统回收，然后界面又重新被启动系统保存的参数；
     * 也有可能是其他使用者带过来的，具体实现交给子类
     *
     * @param args
     */
    protected void parseArgs(Bundle args) {
        ...
        mIsCancelable = args.getBoolean(EXTRA_DIALOG_CANCEL_ABLE_KEY);
    ...
    protected static void putTitleParam(Bundle bundle, String title) {
        bundle.putString(EXTRA_DIALOG_TITLE_KEY, title);
    }

    @NonNull
    protected static void putCancelableParam(Bundle bundle, boolean cancelable) {
        bundle.putBoolean(EXTRA_DIALOG_CANCEL_ABLE_KEY, cancelable);
    }

    @NonNull
    protected static void putMessageParam(Bundle bundle, String message) {
        bundle.putString(EXTRA_DIALOG_MESSAGE_KEY, message);
    }
